...
uint8_t RF24Mesh::update(){
  ...
  if(!getNodeID()){
	...
	if(type == NETWORK_PING){
		uint16_t fromAddr = getNodeID(*(uint16_t*)network.frame_buffer);
		for(uint8_t i=0; i < addrListTop; i++){
			if(addrList[i].nodeID == fromAddr){
				addrList[i].isAlive = 2;
				break;
			}
		}
	}
...
void RF24Mesh::DHCP(){
  ...
  if(doDHCP){
	...
	if(millis() - pingTimer > 40000){
		pingTimer = millis();
		aliveNum = 0;
		for(uint8_t i=0; i < addrListTop; i++){
			if(addrList[i].isAlive > 1){
				aliveNum++;
				addrList[i].isAlive--;
			}else if(addrList[i].isAlive)
				addrList[i].isAlive--;
		}		
	}
	return; 
