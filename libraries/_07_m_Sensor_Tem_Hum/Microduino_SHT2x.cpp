...
{
  //  TEMPERATURE
  if (requestTemperature() == false)
    return false;
  while (reqTempReady() == false)
  {
    yield();
  };
  if (readTemperature() == false)
    return false;

  //  HUMIDITY
  if (requestHumidity() == false)
    return false;
  while (reqHumReady() == false)
  {
    yield();
  };
  if (readHumidity() == false)
    return false;
  return true;
}
...
{
  if (_requestType != SHT2x_REQ_TEMPERATURE)
    return false;
...
{
  if (_requestType != SHT2x_REQ_HUMIDITY)
    return false;
