...
\brief Header file for the AudioPro library
\remarks comments are implemented with Doxygen Markdown format

*/

#ifndef __AUDIOPRO_H__
#define __AUDIOPRO_H__

// include libraries:
#include "Microduino_AudioProConfig.h"
#include "SPI.h"
...
/** \brief State of the AudioPro device
 *
 * Value of AudioPro::playing_state detailing the current state of the VSdsp.
 ...
 *
 * For use with AudioPro::flush_cancel(flush_m) as to how to flush the
 * VSdsp's buffer.
 *
...
/** \brief Flush After
 *
 * This will cause AudioPro::flush_cancel(flush_m) to flush with endfillbyte
...
/** \brief Don't Flush
 *
 * This will cause AudioPro::flush_cancel(flush_m) NOT to flush with endfillbyte
 ...
 * of AudioPro and accessed by static functions.
 */
extern SdFat sd;
...
class AudioPro {
  public:
    ...
    void end();
    uint8_t vs_init();
	void applyPatch(const uint16_t *, uint16_t);
  ...
  private:
    ...
    static void WriteRegister(uint8_t, uint16_t);
    static uint16_t ReadRegister (uint8_t);
    static uint16_t ReadWRAM(uint16_t);
    static void WriteWRAM(uint16_t, uint16_t);
