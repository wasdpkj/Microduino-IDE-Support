...
ColorLED::ColorLED(uint16_t n, uint8_t p, neoPixelType t) :
...
void ColorLED::updateType(neoPixelType t) {
...
void ColorLED::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t *p,
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t ColorLED::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Convert separate R,G,B,W into packed 32-bit WRGB color.
// Packed format is always WRGB, regardless of LED strand color order.
uint32_t ColorLED::Color(uint8_t r, uint8_t g, uint8_t b, uint8_t w) {
  return ((uint32_t)w << 24) | ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t ColorLED::getPixelColor(uint16_t n) const {
...
uint8_t *ColorLED::getPixels(void) const {
  return pixels;
}

uint16_t ColorLED::numPixels(void) const {
  return numLEDs;
}
...
void ColorLED::setBrightness(uint8_t b) {
...
uint8_t ColorLED::getBrightness(void) const {
  return brightness - 1;
}
